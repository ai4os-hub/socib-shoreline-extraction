...
class PatchReconstructor:
    ...
    def combine_patches(
        ...
        if method == "avg":
            reconstruded = PatchReconstructor.combine_patches_avg(
                output, n_classes, orig_h, orig_w, patch_size, stride
            )
        elif method == "max":
            reconstruded = PatchReconstructor.combine_patches_max(
                output, n_classes, orig_h, orig_w, patch_size, stride
            )
        else:
        ...
        reconstruded = reconstruded[
            :,
            padding["top"] : orig_h - padding["bottom"],
            padding["left"] : orig_w - padding["right"],
        ]

        return reconstruded

    @staticmethod
    def combine_patches_avg(
        output: torch.Tensor,
        n_classes: int,
        original_heigh: int,
        original_width: int,
